...
function updateMap(map) {
	if (map.lineLayer && map.myMarkers && map.sharedMarkers) {
		map.removeLayer(lineLayer);
		map.removeLayer(myMarkers);
		map.removeLayer(sharedMarkers);
	}
	function onUpdate(serverLocations, status) {
		var myMarkers = new OpenLayers.Layer.Markers("My Location Markers");
		...
		while (i < Number(obj.locations.length)) {
			...
			myMarkers.addMarker(new OpenLayers.Marker(new OpenLayers.LonLat(
					toDegree(obj.locations[i].long),
		...
		var sharedMarkers = new OpenLayers.Layer.Markers(
		...
		$.each(sharedLocations, function(key, value) {
			sharedMarkers.addMarker(new OpenLayers.Marker(
					new OpenLayers.LonLat(toDegree(value.long),
